    ...
    <main className="grid min-h-screen bg-slate-50 px-4 py-8">
      <div className="mx-auto grid w-full max-w-md content-center">
        <div className="mb-8 flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-slate-950">Campus Board</h1>
            <p className="text-sm text-slate-500">時間割と課題を毎日チェック</p>
          </div>
        </div>

        <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
          <div className="mb-5 grid grid-cols-2 rounded-lg bg-slate-100 p-1">
            <button type="button" onClick={() => setMode("login")} className={`min-h-11 rounded-md text-sm font-semibold ${mode === "login" ? "bg-white text-blue-700 shadow-sm" : "text-slate-500"}`}>
              ログイン
            </button>
            <button type="button" onClick={() => setMode("signup")} className={`min-h-11 rounded-md text-sm font-semibold ${mode === "signup" ? "bg-white text-blue-700 shadow-sm" : "text-slate-500"}`}>
              新規登録
            </button>
          </div>
