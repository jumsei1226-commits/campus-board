...
- 共有ONの時間割テンプレートだけ他ユーザーも閲覧できるポリシー

既存環境に反映する場合は、Supabase SQL Editorで `supabase/schema.sql` を再実行してください。既存の時間割と課題は、ユーザーごとのデフォルト学期 `2026前期` に自動で紐づけられます。
...
- 通知予定課題の判定ロジック
- ダッシュボードへの通知予定課題表示
- ブラウザ通知ON/OFFと通知許可リクエスト
- 時間割テンプレートの作成、共有、詳細表示、コピー
...
- [ ] 課題を追加できる
- [ ] 課題を完了にできる
- [ ] 学期を切り替えると時間割、課題、ダッシュボードの表示が切り替わる
- [ ] 設定ページで二学期制/クォーター制を変更しても既存データが消えない
- [ ] 土曜日表示をONにすると土曜日列が表示され、OFFにすると隠れる
- [ ] 通知ONにしてブラウザ通知を許可できる
- [ ] 締切3日前、前日、当日の課題が目立つ表示になる
- [ ] ダッシュボードに通知予定の課題が表示される
- [ ] 自分の時間割からテンプレートを作成できる
- [ ] 共有ONのテンプレートを一覧と詳細で見られる
- [ ] テンプレートを現在選択中の学期へコピーできる
- [ ] 別ユーザーでログインしたとき、他人の時間割や課題が見えない
- [ ] 別ユーザーは共有ONテンプレートだけ閲覧でき、編集・削除できない
